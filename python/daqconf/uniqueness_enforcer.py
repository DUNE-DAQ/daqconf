...
class UniquenessInformantGroup:
    '''
    Group of uniqueness informants to manage combining and reporting
    '''
    ...
    def combine_identical_objects(self):
        '''
        Combine enforcers that refer to the same DAL id/class
        '''
...
class EnforceredNameGenerator(DalNameGeneratorBase):
    ...
    def __init__(self, dal, config: Configuration, enforcer: UniquenessInformantGroup):
        ...
        if hasattr(dal, "__old_id") and dal.id != dal.__old_id:
            raise ValueError(f"DAL {dal.className()}, {dal.id} has already been renamed from {dal.__old_id}, cannot rename again.")
        ...
        if self._enforced is None:
            raise ValueError(f"DAL {dal.className()}, {dal.id} not found in enforcer list.")

        if dal.id in self._enforced.names:
            raise RenamedDalError(f"DAL {dal.className()}, {dal.id} already has a name assigned in enforcer.")
        ...
        try:
            self.name_index = self._enforced.contained_in_files.index(Path(contained_in))
        except ValueError:
            raise ValueError(f"DAL {dal.className()}, {dal.id} contained in file {contained_in} not found in enforcer's contained_in_files.")

    def dal_in_enforcer(self, dal_obj) -> Optional[UniquenessInformant]:
        for info in self.enforcer.get_enforcers():
            if (info.dal_id == dal_obj.id or dal_obj.id in info.names) and info.dal_class == dal_obj.className():
                return info
...
class GroupNameGenerator(EnforceredNameGenerator):
    ...
    def generate_name(self) -> str:
        ...
        for rel in relations:
            ...
            warnings.warn(f"Could not find relationship linking {related_dal.className()}, {related_dal.id} to {self.dal.className()}, {self.dal.id}.")
    ...
    def _add_to_name_map(self, idx: int, name_base_str: str, dal):
        ...
        if dal_enforcer_instance is None:
            ...
            new_info = UniquenessInformant(dal_id=dal.id, dal_class=dal.className(), names=[name])
            ...
            try:
                file_index = dal_enforcer_instance.contained_in_files.index(Path(contained_in))
                ...
                dal_enforcer_instance.session_files.append(self._enforced.session_files[self.name_index])
...
class PromptNameGenerator(EnforceredNameGenerator):
    ...
    def generate_name(self) -> str:
        ...
        console.print(f"Renaming DAL [bold]{self.dal.className()}, {self.dal.id}[/bold] found in file [bold]{self.config.get_obj(self.dal.className(), self.dal.id).contained_in()}[/bold].")
        if related_dal:
            console.print(f"Object is referenced by: [bold]{related_dal.className()}, {related_dal.id}[/bold].")
        ...
        while True:
            ...
            confirm = Prompt.ask(f"Confirm renaming [bold]{self.dal.className()}, {self.dal.id}[/bold] to [bold]{new_name}[/bold]? (y/n)", choices=["y", "n"], default="y")
            if confirm.lower() == 'y':
                return new_name
            else:
                console.print("Renaming cancelled by user, please try again.", style="bold red")
...
class RenameEngine:
    '''
    Engine to rename DALs based on a given scheme
    '''
    ...
    def apply_scheme(self, scheme: str, enforcers: UniquenessInformantGroup):
        ...
        for enforcer in enforcers.get_enforcers():
            for config_name in enforcer.session_files:
                ...
                if config is None:
                    self._CONFIG_CACHE[config_name] = config = Configuration(f"oksconflibs:{config_name}")
                ...
                except RenamedDalError as _:
                    continue

                except Exception as e:
                    console = Console()
                    console.print(f"[bold red]Failed to rename DAL {dal.className()}, {dal.id} in config {config_name}:[/bold red] {e}")

        # After renaming, we combine identical objects again to update names
...
class ConfigLoader:
    ...
    def __call__(self) -> UniquenessInformantGroup:
        ...
        if Prompt.ask("Do you want to rename conflicting DALs to enforce uniqueness? (y/n)", choices=["y", "n"], default="n")=="n":
            return enforcer
        ...
        while True:
            rename = Prompt.ask(f"Enter renaming scheme ({','.join(c for c in choices)}):", choices=choices, default="prompt", case_sensitive=False)
    ...
    def _display_help(self, rename_engine: RenameEngine):    
        ...
        for choice in rename_engine.recognised_schemes():
            console.print(f" - [bold green]{choice}[/bold green] [cyan]{rename_engine.get_help_message(choice)}[/]")
        
        console.print(" - [bold green]help[/bold green]: [cyan]Display this message.[/]")
...
class UniquenessEnforcerContextManager:
    ...
    def commit(self):
        ...
        if Prompt.ask("Do you want to commit the changes to the configuration files? (y/n)", choices=["y", "n"], default="n") == "y":
            if not self.output_csv:
                Console().print("[bold yellow] Warning: No output CSV specified. In order to track the changes we will generate a csv to uniqueness_report.csv .[/bold yellow]")
