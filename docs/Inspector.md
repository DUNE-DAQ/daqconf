...
Commands:
  ...
  show-excludeable-entities         Show the status of an ExcludableEntity and its contained ones
...
```
Usage: daqconf_inspector CONFIG_FILE show-excludable-entities [OPTIONS] SESSION_ID
                                                    EXCLUDABLE_ENTITY_ID
...
|**Example**|
|---|
|`daqconf_inspector /ehn1-daqconfigs/sessions/hd-coldbox-session.data.xml show-excludable-entities   hd-coldbox-session hd-coldbox-segment`|
|![show-excludable-entities example](./img/inspector_show-resources.png)|
