...
The tools to generate these configurations consist of a single Python script that generates MiniDAQ system configurations with different characteristics based on command-line parameters that are given to the script. This script is daqconf/python/daqconf/scripts/daq_multiru_gen.py.
...
`python -m daqconf.scripts.daq_multiru_gen -h`
...
`python -m daqconf.scripts.daq_multiru_gen -d ./frames.bin -o .  daq_fake`
...
`python -m daqconf.scripts.daq_multiru_gen -d ./frames.bin -o . --trigger-rate-hz 2.0  daq_fake`

`python -m daqconf.scripts.daq_multiru_gen -d ./frames.bin -o . --hsi-event-period 500000000.0 daq_fake`
...
`python -m daqconf.scripts.daq_multiru_gen -d ./frames.bin -o . -s 10  daq_fake`


5) Use option `-r FLOAT` to specify the run number, for example with run 111 :

`python -m daqconf.scripts.daq_multiru_gen -d ./frames.bin -o . -r 111  daq_fake`
...
`python -m daqconf.scripts.daq_multiru_gen -d ./frames.bin -o . -n 4 daq_fake`
...
`python -m daqconf.scripts.daq_multiru_gen -d ./frames.bin -o . --host-df 127.0.0.1 --host-ru 127.0.0.2 --host-trigger 127.0.0.3 --host-hsi 127.0.0.4  daq_fake`
