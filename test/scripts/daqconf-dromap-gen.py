...
import dunedaq.daqconf.dromap as drom
...
e_f = drom.DROStreamEntry(
    ...
    tech="flx", 
    config=drom.FelixStreamConf(host = 'np04-srv-028')
)
...
e_e0 = drom.DROStreamEntry(
    ...
    tech="eth",
    config=drom.EthStreamConf(rx_host = 'np04-srv-029', tx_host='np04-wib-501')
)
...
for e in [e_f, e_e0, e_e1, e_e2]:
    ...
    print(d.tech)
    if d.tech == "flx":
        d.config = drom.FelixStreamConf(**i)
    elif d.tech == "eth":
        d.config = drom.EthStreamConf(**i)
    dro_links.append(d)
